...
namespace Nintenlord.Utility
{
    ...
    public static class CanCauseErrorHelpers
    {
        ...
        public static CanCauseError<T> Assert<T>(T value, Func<T, bool> predicate)
        {
            if (predicate(value))
            {
                return value;
        ...
        public static CanCauseError<T> RenameError<T>(this CanCauseError<T> error, string newErrorMessage)
        {
            if (error.CausedError)
            {
                return CanCauseError<T>.Error(newErrorMessage);
            }
            else
            {
                return error;
